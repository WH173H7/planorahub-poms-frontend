...
function CommunicationSettings(){return <div className="settings-pro-grid"><SettingLink title="PlanoraHub Mail" description="Resend-powered CRM mailbox, drafts and HTML templates." href="/email" action="Open Mail"/><MessengerSetting/><SettingLink title="Broadcast Center" description="Targeted operational announcements and read tracking." href="/broadcasts" action="Manage broadcasts"/><SettingLink title="Mail templates" description="Manage the branded HTML layouts used by staff email." href="/email" action="Open templates"/></div>}
function MessengerSetting(){return <Card><button type="button" className="settings-pro-link settings-pro-button-link" onClick={()=>window.dispatchEvent(new Event('planorahub:open-messenger'))}><div><span className="eyebrow">Communication</span><h3>PlanoraHub Messenger</h3><p>Direct, Team, Department and company conversations stay available from the floating Messages button across the CRM.</p></div><strong>Open Messenger →</strong></button></Card>}
