...
const channelIcon:Record<string,string>={COMPANY:'✦',TEAM:'◎',DEPARTMENT:'◇',CUSTOM:'#',ADMIN:'◆'};
...
export function FloatingChat(){
  ...
  const loadLists=useCallback(async()=>{
    try{
      const[chats,people]=await Promise.all([listChatChannels(),directContacts()]);
      setChannels(chats);setContacts(people);
    }catch{
      // Messenger remains available even if a polling request briefly fails.
    }
  },[]);

  useEffect(()=>{
    let alive=true;
    void getCurrentCrmUser().then(user=>{if(!alive)return;setMe(user);setAdmin(hasAdministrativeAccess(user));return loadLists()}).catch(()=>{});
  ...
  const conversations=useMemo(()=>{
    ...
    const rows:Array<{key:string;kind:'CHANNEL'|'DIRECT';id:string;title:string;subtitle:string;last:string;time:string;unread:number;type:string}>=[];
    if(filter==='ALL'||filter==='DIRECT'){
      for(const item of contacts)rows.push({key:`D-${item.id}`,kind:'DIRECT',id:item.id,title:`${item.first_name} ${item.last_name}`,subtitle:item.job_title||'Direct message',last:item.last_message||'Start a conversation',time:clock(item.last_message_at),unread:Number(item.unread_count||0),type:'DIRECT'});
    }
    for(const item of channels){
      if(filter==='DIRECT')continue;
      if(filter!=='ALL'&&filter!==item.channel_type)continue;
      rows.push({key:`C-${item.id}`,kind:'CHANNEL',id:item.id,title:item.name.replace(/^Team · |^Department · /,''),subtitle:item.channel_type==='TEAM'?'Team chat':item.channel_type==='DEPARTMENT'?'Department chat':item.channel_type==='COMPANY'?'Company chat':item.channel_type==='ADMIN'?'Admin room':'Channel',last:item.last_message||item.description||'No messages yet',time:clock(item.last_message_at),unread:Number(item.unread_count||0),type:item.channel_type});
    }
    return rows.filter(item=>!q||`${item.title} ${item.subtitle} ${item.last}`.toLowerCase().includes(q)).sort((a,b)=>(b.unread-a.unread)||String(b.time).localeCompare(String(a.time))||a.title.localeCompare(b.title));
  ...
  async function send(){
    ...
    const text=body.trim();setBody('');setSending(true);setError(null);
    try{
      if(selection.kind==='CHANNEL'){await sendChatMessage(selection.id,text,reply?.id);setReply(null)}
      else await sendDirect(selection.id,text);
      await refreshConversation();
    }catch(caught){setBody(text);setError(caught instanceof Error?caught.message:'Message could not be sent.')}
    finally{setSending(false);composeRef.current?.focus()}
  }
  ...
  async function upload(file?:File){
    ...
    try{
      ...
      await refreshConversation();
    }catch(caught){setError(caught instanceof Error?caught.message:'Attachment could not be sent.')}
    finally{setSending(false);if(fileRef.current)fileRef.current.value=''}
  }

  async function openAttachment(id:string,direct=false){
    try{const item=direct?await getDirectAttachment(id):await getChatAttachment(id);window.open(item.url,'_blank','noopener,noreferrer')}
    catch{setError('Attachment could not be opened.')}
  }

  function choose(next:Selection){setSelection(next);setReply(null);setEmoji(false);setError(null)}

  return <>
    <button className="floating-chat-button ph-messenger-launcher" aria-label="Open PlanoraHub Messenger" onClick={()=>setOpen(v=>!v)}>
      <span className="ph-messenger-launcher-icon">✦</span><span>Messages</span>{unread?<b className="chat-unread-badge">{unread>99?'99+':unread}</b>:null}
    </button>

    {open?<section className="ph-messenger-panel" aria-label="PlanoraHub Messenger">
      <header className="ph-messenger-topbar">
        <div className="ph-messenger-title"><span className="ph-messenger-mark">P</span><div><strong>PlanoraHub Messenger</strong><small>{unread?`${unread} unread message${unread===1?'':'s'}`:'Direct, Team and Department conversations'}</small></div></div>
        <div className="ph-messenger-top-actions">{admin?<button type="button" title="Create company channel" onClick={()=>setCreateOpen(true)}>＋</button>:null}<button type="button" aria-label="Close Messenger" onClick={()=>setOpen(false)}>×</button></div>
      </header>

      <div className="ph-messenger-layout" data-has-selection={selection?'true':'false'}>
        <aside className="ph-messenger-rail">
          <div className="ph-messenger-search"><span>⌕</span><input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search conversations"/></div>
          <div className="ph-messenger-filters">{(['ALL','DIRECT','TEAM','DEPARTMENT','COMPANY'] as Filter[]).map(value=><button type="button" key={value} className={filter===value?'is-active':''} onClick={()=>setFilter(value)}>{value==='ALL'?'All':value==='DIRECT'?'Direct':value==='TEAM'?'Teams':value==='DEPARTMENT'?'Departments':'Company'}</button>)}</div>
          <div className="ph-messenger-list">
            ...
            {!conversations.length?<div className="ph-messenger-empty"><span>✦</span><strong>No conversations here</strong><small>Try another filter or search term.</small></div>:null}
          </div>
        </aside>
        ...
        <main className="ph-messenger-main">
          {selection?<>
            <header className="ph-messenger-conversation-head">
              ...
              <span className="ph-messenger-head-avatar">{selection.kind==='DIRECT'?(contact?`${contact.first_name[0]}${contact.last_name[0]}`:'DM'):channelIcon[channel?.channel_type||'CUSTOM']}</span>
              <div><strong>{selection.kind==='DIRECT'?`${contact?.first_name||''} ${contact?.last_name||''}`.trim():channel?.name.replace(/^Team · |^Department · /,'')}</strong><small>{selection.kind==='DIRECT'?(contact?.job_title||'Direct message'):(channel?`${channel.channel_type==='TEAM'?'Team':channel.channel_type==='DEPARTMENT'?'Department':channel.channel_type==='COMPANY'?'Company-wide':'Channel'} · ${channel.member_count||0} members`:'')}</small></div>
          ...
          </>:<div className="ph-messenger-welcome"><span className="ph-messenger-welcome-icon">✦</span><h2>Messages, where the work happens.</h2><p>Chat directly with staff or continue conversations with your Team, Department and company.</p><div><span>Direct messages</span><span>Team rooms</span><span>Department rooms</span><span>Company chat</span></div></div>}
        </main>
      </div>
    </section>:null}
